"""
CHRIST University Claims Management System - COMPLETE VERSION
Fixed: UI visibility, cross-verification logic, name matching with titles
"""
...
# ============================================================================
# VERIFICATION ENGINE - ENHANCED WITH SMART NAME MATCHING
# ============================================================================
...
class VerificationEngine:
    ...
    def normalize_name(name: str) -> str:
        ...
        titles = ['dr', 'mr', 'mrs', 'ms', 'miss', 'prof', 'professor', 'dr.', 'mr.', 'mrs.', 'ms.']
        words = name.split()
    ...
    def names_match(name1: str, name2: str) -> bool:
        ...
        # Split into words and check if main parts match
        words1 = set(norm1.split())
        words2 = set(norm2.split())
        
        # If at least 2 words match (for typical first+last name)
        common_words = words1.intersection(words2)
        if len(common_words) >= 2:
            return True
        
        # If both have only 2 words and any word matches
    ...
    def perform_full_verification(form_data: Dict, documents: Dict) -> Dict:
        """
        Enhanced verification with detailed cross-checking:
        1. Name matching (with title handling)
        2. Account number matching
        3. Date matching (within ±3 days)
        4. Amount matching
        """
        issues = []
        ...
        form_event_date = form_data.get('event_date')  # This is the date from form
        
        # Track what was extracted
        extracted_any_data = False
        
        # ===== CHECK 1: NAME MATCHING WITH PROOF =====
        proof_names = []
        proof_dates = []
            ...
            if proof and proof.get('success'):
                ...
                if proof.get('faculty_name'):
                    ...
                    if form_name and proof_name:
                        if not VerificationEngine.names_match(form_name, proof_name):
                            issues.append({
                                'severity': 'high',
                                'message': f"❌ NAME MISMATCH: Form name '{form_name}' does not match certificate name '{proof_name}'"
                            })
        ...
        # ===== CHECK 2: ACCOUNT NUMBER MATCHING WITH RECEIPT =====
        receipt_accounts = []
        receipt_dates = []
        receipt_amounts = []
            ...
            if receipt and receipt.get('success'):
                ...
                if receipt.get('account_number'):
                    ...
                    if form_account and receipt_acc:
                        ...
                        if norm_form and norm_receipt and norm_form != norm_receipt:
                            issues.append({
                                'severity': 'high',
                                'message': f"❌ ACCOUNT MISMATCH: Form account '{form_account}' does not match receipt account '{receipt_acc}'"
                            })
                            high_severity_issues.append("Account mismatch")
                elif form_account:
                    # Account expected but not found
                    issues.append({
                        'severity': 'high',
                        'message': f"❌ ACCOUNT NOT FOUND: Receipt does not contain account number (expected: {form_account})"
                    })
                    high_severity_issues.append("Missing account")
        ...
        # ===== CHECK 3: DATE MATCHING (Receipt/Invoice date within ±3 days of Proof date) =====
        invoice_dates = []
        invoice_amounts = []
        ...
        if proof_dates and all_financial_dates:
            ...
            if not date_matched:
                issues.append({
                    'severity': 'high',
                    'message': f"❌ DATE MISMATCH: Transaction date is NOT within ±{Config.DATE_VARIANCE_DAYS} days of certificate date. Proof dates: {[d.strftime('%Y-%m-%d') for d in proof_dates]}, Transaction dates: {[d.strftime('%Y-%m-%d') for d in all_financial_dates]}"
                })
                high_severity_issues.append("Date mismatch")
        
        # ===== CHECK 4: AMOUNT MATCHING =====
        ...
        if len(high_severity_issues) == 0 and extracted_any_data:
            ...
            status_message = f'⚠️ {len(high_severity_issues)} critical issue(s) detected - Requires Admin Review'
        else:
            status = 'PENDING_ADMIN'
            status_message = '⚠️ Insufficient data extracted - Requires Admin Review'
        ...
        return {
            ...
            'extracted_data': {
                ...
                'dates': {
                    ...
                    'transaction_dates': [d.strftime('%Y-%m-%d') for d in receipt_dates + invoice_dates]
                },
                'amounts': {
                    'form': form_amount,
...
- ✅ **Financial Documents**: Either Invoice OR Receipt (must contain matching account number)
- ⚠️ **Important**: Account number in receipt must match the account number you entered above
        """)
